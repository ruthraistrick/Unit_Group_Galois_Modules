...
Files of the form `.magma` contain code written in the software package Magma: files of the form `.py` contain code written in Python. Please note that all code is originally written in Magma, the python code is simply a wrapper. 
  ...
  *  `statistical_test.py` is a python file that runs the aforementioned statistical test. It basically runs the same code as the magma file as a wrapper. When running it for the first $n = 1000$ fields, we see that $89.2$% of them fall into the decomposable case.
